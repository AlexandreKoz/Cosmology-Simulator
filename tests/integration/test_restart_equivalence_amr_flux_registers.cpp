...
void applyPendingFluxRestartStep(cosmosim::tests::RestartEquivalenceStepContext& context) {
  ...
  if (context.integrator_state.step_index == 0U) {
    ...
    const std::size_t created = cosmosim::amr::mergeFluxRegistersIntoPendingStore(context.state, entries, pendingOptions(0));
    assert(created == 1U);
  ...
  if (context.integrator_state.step_index == 1U) {
    ...
    const std::size_t created = cosmosim::amr::mergeFluxRegistersIntoPendingStore(context.state, entries, pendingOptions(1));
    assert(created == 0U);
    const auto diagnostics = cosmosim::amr::applyCompletePendingFluxRegistersToSimulationState(
        context.state,
        descriptors,
...
int main() {
  ...
  const auto restart_path = cosmosim::tests::stage8RestartPath("restart_equivalence_amr_flux_registers");
  auto state = makeRestartPendingFluxState("restart_equivalence_amr_flux_registers");
  ...
  auto scenario = cosmosim::tests::makeStage8Scenario(
      std::move(state), integrator_state, std::move(scheduler), std::move(output_state), restart_path, 4, 1);
  scenario.step_kernel = applyPendingFluxRestartStep;
  ...
  assert(result.direct_state.gas_cells.density_code[1] == result.restarted_state.gas_cells.density_code[1]);
