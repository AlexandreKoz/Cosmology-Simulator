...
int main() {
  ...
  scheduler.activateBin(0, 0, 0.0);

  cosmosim::io::RestartWritePayload payload;
  payload.state = &state;
  payload.integrator_state = &integrator;
  payload.scheduler = &scheduler;
  payload.provenance = cosmosim::core::makeProvenanceRecord("bench_restart", "bench_restart", 0);
  payload.normalized_config_text = "mode=zoom_in\n";
  payload.normalized_config_hash_hex = "0123456789abcdef";
