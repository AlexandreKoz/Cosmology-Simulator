...
- `process_memory_budget_bytes` (default `0`, meaning no total-process ceiling): authoritative composed per-rank DMO process budget when set. The runtime combines live canonical state, retained scheduler state, gravity/tree staging, PM fields, explicit FFT plan arrays, bounded PM/Tree communication, configured output/restart overlap, external reserves, and the process safety margin before large gravity staging allocations. This does not silently change the legacy gravity-only key.
...
- `process_memory_safety_margin_fraction` (default `0`, range `[0,1]`): multiplicative margin applied after known owned bytes plus external reserves. Swap and host overcommit are not part of this budget.
