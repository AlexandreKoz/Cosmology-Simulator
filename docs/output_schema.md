...
- Restart schema version/name are now `cosmosim_restart_v16`, version `16`, because H2 restart files persist cell-local gas velocity lanes and authoritative gas-cell identity records in addition to Stage 8 file-kind metadata, output cadence/counter state, and compact `/restart_diagnostics` audit metadata alongside scheduler/runtime truth.
...
- Restart schema is `cosmosim_restart_v16`; distributed TreePM state is persisted under restart-only
