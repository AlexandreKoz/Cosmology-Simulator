...
| Gas gravitational source/target truth | Owned authoritative leaf rows in `SimulationState::{cells,gas_cells,gas_cell_identity}` | Cell centre + cell mass + stable `gas_cell_id`; fixed/species gas-softening policy from frozen config | Gravity source/target compaction at synchronized force refresh; invalidate on cell/gas-identity generation change | Generic gas particles are compatibility/lineage mirrors only and are excluded from gravity source mass. Parentless cells and multiple cells sharing one lineage parent remain independent gravity sources/targets. Covered coarse cells are excluded in favor of authoritative leaf descendants. |
...
| Long-range PM field validity | `TreePmCoordinator` | `LongRangeFieldValidity` plus transient PM mesh/halo state | Refresh at an integrator force surface; explicit reuse only after full compatibility validation | Reuse requires the explicit physical `source_generation` plus force epoch/operator inputs. Particle decomposition alone does not invalidate fixed FFT-slab ownership, but any source snapshot change does. |
