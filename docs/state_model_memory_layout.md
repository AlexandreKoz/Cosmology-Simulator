...
- `GasCellSidecar`: persistent gas-cell sidecar (`velocity_[xyz]_peculiar`, `density_code`, `pressure_code`, `internal_energy_code`, `temperature_code`, `sound_speed_code`) plus compatibility identity mirrors (`gas_cell_id`, `parent_particle_id`). `parent_particle_id == 0` mirrors an authoritative parentless identity record; it is not a gas-cell ID sentinel. Hydro transient reconstruction gradients are transient scratch, not restart truth. `pressure_code`, `temperature_code`, and `sound_speed_code` are explicitly classified by memory accounting as persistent restart/source/scheduler compatibility caches derived from the thermodynamic state; they are not additional conserved authorities. Removing those lanes requires a versioned restart-schema/performance campaign.
