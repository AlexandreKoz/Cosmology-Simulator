...
- The production directed AMR hydro exchange is interface-scoped. Candidate ranks first exchange compact `AmrPatchPayloadRecord` descriptors, derive actual face-sharing patch pairs, and materialize only the union of requested boundary planes through `DirectedAmrPatchCellPayloadProvider`. The protocol must not build a full `O(N_local_cells)` directed-hydro cell vector. Remote interior rows that were not advertised carry an explicit unavailable mask and cannot be consumed by ghost fill.
- Directed AMR POD transport is structurally classic-MPI-count safe: logical record counts are 64-bit, payload traffic is split into deterministic bounded byte rounds (16 MiB default, never above `INT_MAX`), and receive/preparation readiness is exchanged before payload traffic. Rejecting a legal production payload merely because its total byte count exceeds one classic MPI `int` is not an accepted protocol.
- Directed AMR diagnostics distinguish traffic from residency. `patch_descriptor_bytes` and `patch_cell_payload_bytes` are transferred-byte counters. `patch_cell_send_capacity_high_water_bytes`, `patch_cell_receive_capacity_high_water_bytes`, `patch_cell_retained_capacity_high_water_bytes`, and `communication_workspace_high_water_bytes` report owned vector capacity/simultaneous workspace and are the values eligible for the central memory high-water report.
