...
- Directed AMR POD transport is structurally classic-MPI-count safe: logical record counts are 64-bit, physical payload traffic is split into deterministic record-aligned bounded rounds (16 MiB default, never above `INT_MAX`), and admission/producer/consumer readiness is exchanged before advancing each corresponding payload stage. Rejecting a legal production payload merely because its total byte count exceeds one classic MPI `int` is not an accepted protocol.
