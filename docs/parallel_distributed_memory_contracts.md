...
- The production directed AMR hydro exchange is interface-scoped. Candidate ranks first exchange compact `AmrPatchPayloadRecord` descriptors, derive actual face-sharing patch pairs, and stream only the union of requested boundary planes through `DirectedAmrPatchCellPayloadProvider`. The complete logical cell payload is never required as one send or receive vector.
- Directed AMR POD transport is structurally classic-MPI-count safe: logical record counts are 64-bit, physical payload traffic is split into deterministic record-aligned bounded rounds (16 MiB default, never above `INT_MAX`), and admission/producer/consumer readiness is exchanged before advancing each corresponding payload stage. Round planning itself is constant-space: production keeps only `{logical_record_count, records_per_round, round_count}` for each direction and derives each round arithmetically rather than retaining one metadata entry per logical round. Rejecting a legal production payload merely because its total byte count exceeds one classic MPI `int`, or allocating O(round-count) planner metadata when the governor shrinks the transport window, is not an accepted protocol.
- Received hydro patch-cell records are consumed immediately into compact per-patch sparse boundary stores keyed by patch-local offset. Remote ghost-only patches do not allocate full-volume gas-ID, conserved-state, availability, internal-face, or solver-geometry arrays. Missing required sparse cells are deterministic errors rather than zero/default state.
- Directed AMR diagnostics distinguish traffic from residency. `patch_descriptor_bytes` and `patch_cell_payload_bytes` are transferred-byte counters. `patch_cell_send_capacity_high_water_bytes`, `patch_cell_receive_capacity_high_water_bytes`, and `communication_workspace_high_water_bytes` report physical reusable transport capacity. Hydro runtime accounting separately reports sparse ghost cell capacity and O(remote-patch) metadata; the aggregate communication high-water must cover their simultaneous live set rather than summing traffic volume.
- Hydro remote-ghost transport and sparse-store growth are admitted through the existing `MemoryGovernor`. A bounded MPI round without bounded/governed physical residency is not sufficient for this contract.
