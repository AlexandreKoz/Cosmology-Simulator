...
- The previously failing `integration_reference_workflow` schema-compatibility lane is repaired in the current source tree.
- The previously failing periodic zoom-contamination invariant is repaired by aligning the integration test scenario with the documented contamination-radius contract.
- The previously failing single-rank communication-stress validation expectation is repaired by enforcing remote-communication diagnostics only for true multi-rank execution.
- `power_spectrum_consistency` is no longer blocked: a deterministic checked-in reference floor is generated by `validation_phase3_power_spectrum` and consumed by `collect_phase3_evidence.py` when runtime-heavy diagnostics are absent.
- Multi-rank scaling evidence in the audited artifact set remains baseline-only and non-certifying (`phase2_baseline_scaling_summary.json`); checked-in CSV artifacts still cover `np1` only and explicitly mark missing `np2` artifacts.
