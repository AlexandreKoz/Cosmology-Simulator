...
| P19-PARALLEL-CONTRACT-009 | Closed (R07 hardened) | Distributed-memory ownership/determinism contracts | Owned-vs-ghost residency and reduction/config consensus expectations were partially implicit in ghost-owner vectors and ad-hoc checks. | Added explicit send/receive transfer-role descriptors, stricter ghost plan/buffer invariants, unambiguous reduction baseline/measured naming + tolerance helper, and property-level config mismatch records with rank/value payloads; validated in `test_parallel_distributed_memory` and `test_parallel_two_rank_restart`. |
