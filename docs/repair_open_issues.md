...
| P34-RELEASE-SCHEMA-DRIFT-031 | Closed pending local full-suite certification | Release/docs schema metadata drift | Release manifest, docs scaffold, and release readiness checks now agree on snapshot `gadget_arepo_v4`, restart `cosmosim_restart_v11`, and provenance `provenance_v5`. | Re-run docs scaffold, release readiness, and snapshot/restart schema tests locally. |
