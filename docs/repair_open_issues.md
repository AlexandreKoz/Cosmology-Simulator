...
| --- | --- | --- | --- | --- |
| MEMORY-GOVERNOR-FOUNDATION-20260829 | Closed for M1B foundation | Process memory admission | One process governor, move-only reservations, pressure/headroom/high-water telemetry, no-double-count baseline reconciliation, production budget ownership, and governed monotonic-scratch block growth are implemented and covered by focused/production smoke tests. | M1B does not claim broad large-allocation coverage or whole-process measured-memory closure. |
