...
| ID | Status | Area | Resolution in this pass | Remaining boundary |
| --- | --- | --- | --- | --- |
| CORE-SCHEDULER-ORDERED-ACTIVE-20260808 | Open performance follow-up | Hierarchical scheduler | Exhaustive invariant scans are debug/audit-only in hot begin/end-substep paths; correctness and deterministic active order are preserved. | Active indices still require a deterministic sort because current bin membership uses swap-with-last removal. Replace only with an ordered/incrementally maintained structure backed by benchmark evidence. |
| CORE-MIGRATION-PEAK-RSS-20260808 | Open performance follow-up | Migration memory | Sidecar destination rows are pre-counted/resized once and host remaps use prepare/validate/commit semantics. | Old and rebuilt authoritative state intentionally coexist until commit. Quantify peak RSS on representative large migrations before adopting chunked rebuilds that could weaken transactionality. |
| CORE-PROVENANCE-V7-INTEGRITY-20260808 | Open schema follow-up | Provenance | Provenance parsing is strict, duplicate/unknown keys fail closed, finite numerics are required, and final publication is staged through `.part`. | `provenance_v6` retains its established FNV-1a compatibility hash and limited hardware summary. Introduce SHA-256/BLAKE3 plus richer CPU/RAM/GPU/MPI metadata only through an explicitly versioned compatible schema migration. |
